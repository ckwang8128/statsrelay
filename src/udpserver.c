...
static udplistener_t *udplistener_create(udpserver_t *server, struct addrinfo *addr, int (*cb_recv)(int, void *)) {
	...
	int rcv_buffer_size = 805306368; // cat /proc/sys/net/core/rmem_max
	...
	stats_debug_log("new udp listening socket receive buf %d\n", new_rcv_buffer_size); // e.g. 805306368 or twice the size?
	if (new_rcv_buffer_size < rcv_buffer_size) {
		stats_log("udplistener: Error setting SO_RCVBUF on %s[:%i]: %s; did you: sysctl -w net.core.rmem_max=805306368 # 768 MB", addr_string, port, strerror(errno));
		free(listener);
		return NULL;
	}
